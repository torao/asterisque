...
import io.asterisque.util.Latch;
...
class PipeOutputStream extends OutputStream {
  ...
  private void ensureWrite(int len) throws IOException {
    if(closed){
      throw new IOException(pipe + ": unable to write to closed pipe or stream: " + pipe.id);
    }
  ...
  public void emergencyClose() {
    closed = true;
    logger.trace(pipe + ": output stream closed by peer");
  }
}

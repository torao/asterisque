...
class PipeSpace {
  ...
  PipeSpace(Session session) {
    ...
    this.pipeMask = session.isPrimary ? Pipe.UniqueMask : 0;
  }
  ...
  public Pipe create(@Nonnull Open open) throws ProtocolException {
    ...
    boolean hasPrimaryMask = (open.pipeId & Pipe.UniqueMask) != 0;
    ...
    // 新しいパイプを構築して登録
    Pipe pipe = new Pipe(open.pipeId, open.priority, open.functionId, session);

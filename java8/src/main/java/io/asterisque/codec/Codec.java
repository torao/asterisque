...
public interface Codec {
	 ...
	 */
	public static int MaxFieldsInStruct = 0xFF;
	 ...
	 * 変換後のバイナリサイズが {@link #MaxMessageSize} を超える場合には {@link io.asterisque.codec.CodecException}
	 * が発生します。
	 * @throws io.asterisque.codec.CodecException シリアライズに失敗した場合
	 */
		...
		if(msg instanceof Open) {
			...
			if(block.eof) {
				...
				if(block.length > 0xFFFF){
					throw new CodecException(String.format("block length too large: %d", block.length));
				}
				m.writeTrue();
	...
	public interface Marshal {
		...
		// ============================================================================================
		/**
		 * Asterisque がサポートする任意のデータを {@link io.asterisque.codec.Codec.Tag タグ} 付きで書き込み
		 * ます。このメソッドは {@link io.asterisque.codec.Codec.Unmarshal#read()} の対になるメソッドです。
		 */
	 ...
	 * 実装クラスは対象データを復元できるだけのバイナリを受信していない場合に {@link io.asterisque.codec.Codec.Unsatisfied}
	 * を throw する必要があります。
	 * <p>
	 ...
	 * {@link io.asterisque.codec.Codec.Unsatisfied} が throw される場合は呼び出し側で位置のリセットが行われ
	 * るため、サブクラス側でバッファ位置を復元する必要はありません。
	 * <p>
	 * 各メソッド Unsatisfied の他にメッセージのデコードに失敗した場合は {@link io.asterisque.codec.CodecException}
	 * が発生します。
	 *
	...
	public interface Unmarshal {
		...
		// ============================================================================================
		/**
		 * 先行する {@link io.asterisque.codec.Codec.Tag タグ} で識別される、Asterisque がサポートする任意の
		 * データを読み込みます。このメソッドは {@link io.asterisque.codec.Codec.Marshal#write(Object)} の
		 * 対になるメソッドです。
		 */
	...
	// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	/**
	 * 読み込まれているバイナリがデータを復元するために足りない場合に {@link io.asterisque.codec.Codec.Unmarshal}

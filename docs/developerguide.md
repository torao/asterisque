...
Asterisque は非同期 I/O の難解さをうまくラップしインターフェースの共有による手軽で型安全な同期/非同期 RPC、非同期メッセージパッシング、データストリーミングを共存実装できるよう慎重に設計しています。

現実的なアプリケーション実装においては `Future` や `ExecutionContext` 等の理解が必要となるため、実装者は Scala での非同期処理や並行処理に関する基本な知識を持っている事を前提としています。

### 構成の概要

まず Asterisque の基本的な構成と各コンポーネントの役割について説明します。
